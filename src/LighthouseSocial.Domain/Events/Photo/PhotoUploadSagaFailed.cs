...
public class PhotoUploadSagaFailed(Guid photoId, Guid sagaId, string fileName, Guid userId, Guid lighthouseId, string cameraType, string resolution, string lens, string errorMessage, string? errorDetails = null, string? failedStep = null)
    : EventBase(photoId)
{
    ...
    public string? FailedStep { get; } = failedStep;
